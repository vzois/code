...
from heapq import nsmallest
from hashSkyline import hSkyline
import math
import operator
import time
...
def bit_vectors(gsky_i):
    ...
    for v in bv:
        #hx = pfmtb(v)
        hx = format(v, '08x')
        addr = "0x"+format(mramAddr,"08x")
        ss = str(addr)+ " | " +(hx[0:4]+"."+hx[4:])
        #print ss
        fp.writelines(ss)
        fp.write("\n")
        mramAddr+=4
...
def dskyline():
    ...
    for i in range(len(parts_p)):
        ...
        #if (part_index < 32 and (len(gsky_i) > 0)) or True:#debugging data
        if (part_index < 128):
...
if False:
    ...
    print "sfs:",sky_sfs
    hSky=hSkyline(points)
    for p in sky_sfs:
        if p not in hSky:
            print p," Missing------------------------>"
    
    print "hSky len:",len(hSky), hSky       
else:
